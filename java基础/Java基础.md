...
| Shell | O($nlogn$)~ O($n^2$) | O(n1.3​) | O($n^2$) | O(1)  | 不稳定 |
...
Spring对Java配置的支持是由@Configuration注解和@Bean注解来实现的。由@Bean注解的方法将会实例化、配置和初始化一个新对象，这个对象将由Spring的IOC容器来管理。@Bean声明所起到的作用与<bean/> 元素类似。被**@Configuration**所注解的类则表示这个类的主要目的是作为bean**定义的资源**。被@Configuration声明的类可以通过在同一个类的内部调用@bean方法来设置嵌入bean的依赖关系。
...
上下文停止事件（ContextStoppedEvent）：当容器调用Configurabl eApplicationContext的Stop()方法停止容器时触发该事件。
...
**step3**：**转发rd.forward(request,response);//JSP和Servlet会共享相同的请求和响应对象
...
Ø request 用户端请求，此请求会包含来自GET/POST请求的参数 

Ø response 网页传回用户端的回应 

Ø pageContext 网页的属性是在这里管理 

Ø session 与请求有关的会话期 

Ø application servlet 正在执行的内容 

Ø out 用来传送回应的输出 

Ø config servlet的构架部件 

Ø page JSP网页本身 

Ø exception 针对错误网页，未捕捉的例外 

## 4. jsp有哪些动作?作用分别是什么? ★★★★★

JSP共有以下6种基本动作 

Ø jsp:include：在页面被请求的时候引入一个文件。 

Ø jsp:useBean：寻找或者实例化一个JavaBean。 

Ø jsp:setProperty：设置JavaBean的属性。 

Ø jsp:getProperty：输出某个JavaBean的属性。 

Ø jsp:forward：把请求转到一个新的页面。 

Ø jsp:plugin：根据浏览器类型为Java插件生成OBJECT或EMBED标记 

## 5. JSP中动态INCLUDE与静态INCLUDE的区别？ 

**动态**INCLUDE用jsp:include动作实现 

<jsp:include page="included.jsp" flush="true" />它总是会检查所含文件中的变化，适合用于包含动态页面，并且可以带参数 

**静态**INCLUDE用include伪码实现,定不会检查所含文件的变化，适用于包含静态页面 
...
a) Servlet容器加载servlet类，把它的**. Class**文件中的数据读到内存中。

b) Servlet容器创建servletConfig对象。servletConfig对象包含了servlet的初始化配置信息。此外servlet容器还会使得servletConfig对象与当前的web应用的servletContext对象关联。

c) Servlet容器创建servlet对象。

d) Servlet容器调用servlet对象的init(ServletConfig  config)方法。

通过初始化步骤，**创建了**servlet**对象和servletConfig**对象，并且servlet**对象与servletConfig**对象关联，而servletConfig**对象又与当前对象的servletContext**对象关联**。当servlet容器完成servlet后，servlet对象只要通过getServletContext()方法就能得到web应用的servletContext对象。
...
在这个阶段sevlet可以随时响应客户端的请求。当servlet容器接到访问特定的servlet请求时，servlet容器会创建**针对**与这个请求的servletRequest和servletResponse对象，然后**调用**service()**方法**，并把这两个对象当做参数传递给service()方法。Service()方法通过servletRequest对象获得请求信息，并处理该请求，再通过servletResponse对象生成响应结果。
...
## 12. Servlet API的两个主要包

javax.servlet.*;

javax.servlet.http.*;

## 13. 编写Servlet需要继承的类是什么？

HttpServlet

## 14. 编写Servlet通常需要实现的两个方法是什么？

doGet方法和doPost方法

## 15. doGet方法和doPost方法中的两个参数是什么？
...
## 20. 在Servlet中如何获取Session对象，如何获取Cookie？
...
## 22. 如何实现servlet的单线程模式？

<%@ page isThreadSafe="false"%>

## 23. 我们没有写servlet的构造方法，那么容器是怎么创建servlet的实例呢?
...
Ø 客户端(很可能是Web浏览器)通过HTTP提出请求。

Ø Web服务器接收该请求并将其发给servlet。如果这个servlet尚未被加载，Web服务器将把它加载到Java虚拟机并且执行它。

Ø servlet将接收该HTTP请求并执行某种处理。

Ø servlet将向Web服务器返回应答。

Ø Web服务器将从servlet收到的应答发送给客户端。
...
Ø servlet是与平台无关的。如前所述，servlet是用Java编写的，它自然也继承了Java的平台无关性。

Ø servlet是可扩展的。由于servlet是用Java编写的，它就具备了Java所能带来的所有优点。Java是健壮的、面向对象的编程语言，它很容易扩展以适应你的需求。servlet自然也具备了这些特征。

Ø servlet是安全的。从外界调用一个servlet的惟一方法就是通过Web服务器。这提供了高水平的安全性保障，尤其是在你的Web服务器有防火墙保护的时候。

Ø setvlet可以在多种多样的客户机上使用。由于servlet是用Java编写的，所以你可以很方便地在HTML中使用它们，就像你使用applet一样。

## 26. servlet容器对url的匹配过程：
...
Ø 以”/’开头和以”/*”结尾的是用来做路径映射的。

Ø 以前缀”*.”开头的是用来做扩展映射的。

Ø “/” 是用来定义default servlet映射的。

Ø 剩下的都是用来定义详细映射的。比如： /aa/bb/cc.action

Ø 所以，为什么定义”/*.action”这样一个看起来很正常的匹配会错?因为这个匹配即属于路径映射，也属于扩展映射，导致容器无法判断。

## 27. filter 的作用是什么?主要实现什么方法?doFilter方法里面的2个参数request 和 response 这两个接口的全称是什么?
...
## 28. 如何防止在jsp/servlet中的输出不被browser保存在cache中？
...
如何得到RequestDispatcher，有三种方法可以得到Request Dispatcher对象。
...
## 32. 在servlet和jsp之间能共享session对象吗？
...
Ø 创建并返回一个包含基于客户请求性质的动态内容的完整的 HTML 页面。

Ø 创建可嵌入到现有 HTML 页面中的一部分 HTML 页面（ HTML 片段）。

Ø 与其它服务器资源（包括数据库和基于 Java 的应用程序）进行通信。

Ø 用多个客户机处理连接，接收多个客户机的输入，并将结果广播到多个客户机上。例如， Servlet 可以是多参与者的游戏服务器。

Ø 当允许在单连接方式下传送数据的情况下，在浏览器上打开服务器至 applet 的新连接，并将该连

Ø 接保持在打开状态。当允许客户机和服务器简单、高效地执行会话的情况下， applet 也可以启动客户浏览器和服务器之间的连接。可以通过定制协议或标准（如 IIOP ）进行通信。

Ø 对特殊的处理采用 MIME 类型过滤数据，例如图像转换和服务器端包括（ SSI ）。

Ø 将定制的处理提供给所有服务器的标准例行程序。例如， Servlet 可以修改如何认证用户。